$(document).ready(function () {
    var table = $('#soldpropertymasterlist').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": "/admin/soldpropertymasterlist/getData",
            "type": "GET"
        },
        "columns": [
            { "data": "property_name" },
            ...
            { "data": "publishstatus" },
            {
                ...
                "render": function (data, type, row) {
                    return `
                        ...
                        <a href="#" title="Unsold" class="sold-btn" data-id="${row.property_id}" style="color: green;">
    ...
    $(document).on('click', '.sold-btn', function (e) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: "You are about to revert this property into unsold!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Reverted to unsold!',
            cancelButtonText: 'No, cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                // Make an AJAX request to mark the property as sold
                $.ajax({
                    url: '/admin/soldpropertymasterlist/unSold',  // Your backend URL to handle the sold action
                    type: 'POST',
                    ...
                    success: function (response) {
                        if (response.success) {
                            Swal.fire(
                                'Unsold!',
                                'The property has been reverted to unsold.',
                                'success'
                            );
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/admin/soldpropertymasterlist/delete/' + id,
                    method: 'DELETE',
                    success: function (response) {
    ...
    $(document).on('click', '.propertyDetails', function () {
        ...
        $.ajax({
            type: "GET",
            url: "/admin/soldpropertymasterlist/propertyDetails",
    ...
    function deleteImage(imageId, button) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/admin/soldpropertymasterlist/deleteImage/' + imageId,
                    method: 'DELETE',
                    success: function (response) {
                        if (response.status === 'success') {
